...
from publico.handlers import endpoint_posts
from publico.handlers import endpoint_offers

fake_data = Faker()

'''----------- Test create offer success --------------------'''

@patch('publico.core.endpoint_posts')
@patch('publico.core.endpoint_users')
@patch('publico.core.endpoint_offers')
...
def test_crear_oferta(mock_endpoint_offers,
    ...
    response = app.test_client().post('/public/posts/1/offers', json=data_create, headers={"Authorization": f'Bearer {token}'})
...
def test_crear_oferta_mal_token(mock_endpoint_offers,
    ...
    response = app.test_client().post('/public/posts/1/offers', json=data_create, headers=token)
...
def test_crear_oferta_post_no_existe(mock_endpoint_offers,
    ...
    response = app.test_client().post('/public/posts/10/offers', json=data_create, headers={"Authorization": f'Bearer {token}'})
