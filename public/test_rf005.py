...
@patch('publico.core.endpoint_journey')
...
def test_consultar_publicacion(mock_endpoint_offers,
    ...
    response = app.test_client().get('/public/posts/1', json=data, headers={"Authorization": f'Bearer {token}'})
    response_info = json.loads(response.data.decode('utf-8'))
    print(response_info)
    assert response.status_code == 200
    assert response_info['data']['createdAt'] == "2023-02-23T19:29:31.709490"
    assert len(response_info['data']['offers']) > 1
